...
INTEGER (KIND=JPIM) :: ITIME, INHARM
...
DO INHARM = 1,NSPEC2
  ...
  IF (LELAM) THEN
    ...
    ZSPBUFL (:, :)=0.0_JPRB
    ZSPBUFL (1, INHARM) = 1.0_JPRB

    CALL EINV_TRANS (PSPSCALAR=ZSPBUFL, PGP=ZGPBUFL, KVSETSC=IVSET)
    CALL EDIR_TRANS (PSPSCALAR=ZSPBUFL, PGP=ZGPBUFL, KVSETSC=IVSET)

    PRINT *, "INHARM = ", INHARM, COUNT (ABS (ZSPBUFL) < 1E-14), COUNT (ABS (ZSPBUFL-1.) < 1E-14)

#ifdef UNDEF
BLOCK
REAL (KIND=JPRB) :: ZZSPBUFL (NSPEC2G)

WHERE (ABS (ZSPBUFL (1, :)) < 1E-14)
  ZZSPBUFL = 0.
ELSEWHERE
  ZZSPBUFL = ZSPBUFL (1, :)
ENDWHERE

WRITE (CLFILE, '("ZSPBUFL.",I6.6,".txt")') INHARM
OPEN (77, FORM="FORMATTED", FILE=CLFILE)
WRITE (77, '(10E25.15)') ZZSPBUFL
CLOSE (77)
ENDBLOCK
#endif
...
WRITE(CLFILE, '("aatestprog.20x18.gp.",I6.6,".dat")') INHARM
OPEN (UNIT=8, FILE=CLFILE, FORM="UNFORMATTED")
WRITE (8) ZGPBUFL
CLOSE (8)
